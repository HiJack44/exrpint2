...
class Spinbox(ctik.CTkFrame):
    def __init__(
        ...
        **kwargs
        ...
        self.substract_button = ctik.CTkButton(
            self,
            width=15,
            text="-",
            command=lambda master=self: Spinbox.substract_number(master),
        )
        self.substract_button.grid(row=0, column=0, padx=0, pady=0)

        # Add button adds number
        self.add_button = ctik.CTkButton(
            self,
            width=15,
            text="+",
            command=lambda master=self: Spinbox.add_number(master),
        )
